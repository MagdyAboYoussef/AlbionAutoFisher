...
left_region = (y_top, x_top+30, 150, y_bot-y_top)
right_region = (x_bottom-150, x_top+30, 150, y_bot-y_top)
bottom_region = (x_top, y_bot-155, x_bottom-x_top, 160)
top_region = (x_top+150, x_top+30, x_bottom-x_top-150, 45)
...
threshold = 0.9
#cv2.imshow('test',np.array(pyautogui.screenshot(region=bottom_region)))
#cv2.waitKey()
while True:
    ...
    def match_template(region, result, player_image):
        ...
        result[0] = cv2.matchTemplate(gray, player_image, method)
    ...
    max_vals = [result[0].max() for result in results]
